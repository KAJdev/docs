...
description: "When it comes to working with an AI image generator, the speed in which images are generated is often a compromise. RunPod's Serverless Workers allows you to host  from Stability AI, which is a fast text-to-image model."
---
...
### RunPod[​](#runpod "Direct link to RunPod")
...
## Build and push your Docker image[​](#build-and-push-your-docker-image "Direct link to Build and push your Docker image")
...
## Send a request[​](#send-a-request "Direct link to Send a request")
...
```
curl -X POST "https://api.runpod.ai/v2/${YOUR_ENDPOINT}/runsync" \     -H "accept: application/json" \     -H "content-type: application/json" \     -H "authorization: ${YOUR_API_KEY}" \     -d '{        "input": {            "prompt": "${YOUR_PROMPT}",            "num_inference_steps": 25,            "refiner_inference_steps": 50,            "width": 1024,            "height": 1024,            "guidance_scale": 7.5,            "strength": 0.3,            "seed": null,            "num_images": 1        }     }'
```

```
{  "delayTime": 168,  "executionTime": 251,  "id": "sync-fa542d19-92b2-47d0-8e58-c01878f0365d-u1",  "output": "BASE_64",  "status": "COMPLETED"}
```
...
## Integrate into your application[​](#integrate-into-your-application "Direct link to Integrate into your application")
...
```
<!-- index.html --><!DOCTYPE html><html lang="en"><head>    <meta charset="UTF-8">    <meta name="viewport" content="width=device-width, initial-scale=1.0">    <title>RunPod AI Image Generator</title>    <style>        body {            font-family: Arial, sans-serif;            text-align: center;            padding: 20px;        }        #imageResult {            margin-top: 20px;        }    </style></head><body>    <h1>RunPod AI Image Generator</h1>    <input type="text" id="promptInput" placeholder="Enter your image prompt" />    <button onclick="generateImage()">Generate Image</button>    <div id="imageResult"></div>    <script src="script.js"></script></body></html>
```
...
## Run a server[​](#run-a-server "Direct link to Run a server")
...
[Edit this page](https://github.com/runpod/docs/blob/main/docs/tutorials/serverless/gpu/generate-sdxl-turbo.md)
