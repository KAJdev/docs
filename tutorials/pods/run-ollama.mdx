...
description: "This tutorial will guide you through setting up , a powerful platform serving large language model, on a GPU Pod using RunPod. Ollama makes it easy to run, create, and customize models."
---
...
## Step 1: Start a PyTorch Template on RunPod[​](#step-1-start-a-pytorch-template-on-runpod "Direct link to Step 1: Start a PyTorch Template on RunPod")
...
## Step 2: Install Ollama[​](#step-2-install-ollama "Direct link to Step 2: Install Ollama")
...
## Step 3: Run an AI Model with Ollama[​](#step-3-run-an-ai-model-with-ollama "Direct link to Step 3: Run an AI Model with Ollama")

To run an AI model using Ollama, pass the model name to the `ollama run` command:

```
ollama run [model name]# ollama run llama2# ollama run mistral
```
...
## Step 4: Interact with Ollama via HTTP API[​](#step-4-interact-with-ollama-via-http-api "Direct link to Step 4: Interact with Ollama via HTTP API")
...
```
curl https://{POD_ID}-11434.proxy.runpod.net/api/tags# curl https://cmko4ns22b84xo-11434.proxy.runpod.net/api/tags
```

Replace `[your-pod-id]` with your actual Pod Id.

```
{  "models": [    {      "name": "mistral:latest",      "model": "mistral:latest",      "modified_at": "2024-02-16T18:22:39.948000568Z",      "size": 4109865159,      "digest": "61e88e884507ba5e06c49b40e6226884b2a16e872382c2b44a42f2d119d804a5",      "details": {        "parent_model": "",        "format": "gguf",        "family": "llama",        "families": [          "llama"        ],        "parameter_size": "7B",        "quantization_level": "Q4_0"      }    }  ]}
```
...
```
curl -X POST https://{POD_ID}-11434.proxy.runpod.net/api/generate -d '{  "model": "mistral",  "prompt":"Here is a story about llamas eating grass" }'
```
...
[Edit this page](https://github.com/runpod/docs/blob/main/docs/tutorials/pods/run-ollama.md)
